...
from gym_pursuite.envs.env_settings import PursuitConstants
...
class PursuitGameEnv(gym.Env):
    ...
    def step(self, action): 
        ...
        a = np.zeros(2)
        cpl2 = np.cos(self.player_2[6])
        spl2 = np.sin(self.player_2[6])

        p2_speed_abs = np.sqrt(self.player_2[2]*self.player_2[2]+ self.player_2[3]*self.player_2[3])
        ...
        cpl1 = np.cos(self.player_1[6])
        spl1 = np.sin(self.player_1[6])
        t1a = getAngle(cpl1, spl1, -dd1x, -dd1y)

        p1_alpha = 1.0
        p1_beta = t1a / self.params[4]
        if p1_beta < -1.0:
            p1_beta = -1.0
        if p1_beta > 1.0:
            p1_beta = 1.0
        ...
        a = np.zeros(2)
        p1_speed_abs = np.sqrt(self.player_1[2]*self.player_1[2]+ self.player_1[3]*self.player_1[3])
        ...
        if self.player_1[0] < 0.0: 
            self.player_1[0] = 0.0
            self.player_1[2] = 0.0

        if self.player_1[1] < 0.0: 
            self.player_1[1] = 0.0
            self.player_1[3] = 0.0

        if self.player_1[0] > self.world_shape[0]:
            self.player_1[0] = self.world_shape[0]
            self.player_1[2] = 0.0

        if self.player_1[1] > self.world_shape[1]: 
            self.player_1[1] = self.world_shape[1]
            self.player_1[3] = 0.0

        p1_speed_abs = np.sqrt(self.player_1[2]*self.player_1[2]+ self.player_1[3]*self.player_1[3])
        if p1_speed_abs >= self.params[5]:
            self.player_1[2] = cpl1 * self.params[5]
            self.player_1[3] = spl1 * self.params[5]


        if self.player_2[0] < 0.0: 
            self.player_2[0] = 0.0
            self.player_2[2] = 0.0

        if self.player_2[1] < 0.0: 
            self.player_2[1] = 0.0
            self.player_2[3] = 0.0

        if self.player_2[0] > self.world_shape[0]:
            self.player_2[0] = self.world_shape[0]
            self.player_2[2] = 0.0

        if self.player_2[1] > self.world_shape[1]: 
            self.player_2[1] = self.world_shape[1]
            self.player_2[3] = 0.0

        p2_speed_abs = np.sqrt(self.player_2[2]*self.player_2[2]+ self.player_2[3]*self.player_2[3])
        if p2_speed_abs >= self.params[5]:
            cpl2 = np.cos(self.player_2[6])
            spl2 = np.sin(self.player_2[6])
            self.player_2[2] = cpl2 * self.params[5]
            self.player_2[3] = spl2 * self.params[5]
        ...
        if self.player_1[6] >= 2.0*np.pi:
            self.player_1[6]-= 2.0*np.pi
        elif self.player_1[6] <= -2.0*np.pi:
            self.player_1[6] += 2.0*np.pi

        if self.player_2[6] >= 2.0*np.pi:
            self.player_2[6]-= 2.0*np.pi
        elif self.player_2[6] <= -2.0*np.pi:
            self.player_2[6] += 2.0*np.pi
    ...
    def reset(self): 
        ...
        self.player_1 = np.zeros(8)  # x, y, v_x, v_y, a_x, a_y, phi, omega
        self.player_2 = np.zeros(8)  # x, y, v_x, v_y, a_x, a_y, phi, omegas
    ...
    def render(self, mode='human', close=False):
        ...
        if mode == 'human':
            ...
            phi = -(180.0/np.pi)*self.player_1[6] - 90.0
            ...
            phi = -(180.0/np.pi)*self.player_2[6] - 90.0 -180.0
            im =  pg.transform.rotate(self.player_sprite, phi)
            h = im.get_height()/2.0
            w = im.get_width()/2.0
            im.set_colorkey((0,128,0))
